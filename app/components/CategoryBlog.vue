  ...
  <div class="bg-black py-20 sm:py-28 lg:py-32">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      ...
      <div v-else class="mx-auto mt-12 grid max-w-2xl grid-cols-1 gap-8 border-t border-gray-800 pt-12 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
        <article v-for="post in posts" :key="post._id" class="group flex flex-col bg-white/[0.02] backdrop-blur-sm rounded-2xl border border-gray-800 hover:border-red-900/50 transition-all duration-500 overflow-hidden hover:shadow-2xl hover:shadow-red-950/20 hover:-translate-y-1">
...
const fetchPosts = async () => {
  try {
    ...
    const response = await fetch(getUrl(`blog/?page=1&limit=${props.no_blog_posts}&category_id=${props.category_id}`))
    const data = await response.json()
    posts.value = data.blogs
    category_name.value = data.blogs[0]?.category_name
  } catch (error) {
